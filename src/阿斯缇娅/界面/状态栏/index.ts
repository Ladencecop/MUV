...
function report_error(text: string) {
  ...
  el.id = 'boot-probe';
  el.style.cssText =
    'margin:8px;padding:10px;border:1px solid #b8453f;background:#1a1e26;color:#c9c6bd;font-size:12px;line-height:1.6;white-space:pre-wrap';
  el.textContent = `《阿斯缇娅》状态栏未能渲染：${text}\n请确认酒馆助手已启用，且本楼层已初始化 MVU 变量。`;
  document.body.appendChild(el);
}

$(async () => {
  console.info('[阿斯缇娅 状态栏] 入口脚本已执行');
